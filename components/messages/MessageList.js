...
const MessageList = ({ threads, onSelectThread, selectedThreadId, currentUser, onDeleteThread, usersMap }) => {
    ...
    return (
        <ScrollArea style={{ height: 'calc(100vh - 180px)'}}>
             <Text size="xs" c="dimmed" p="xs">Your inbox shows your conversations.</Text>
            {threads.map((thread) => {
                ...
                return (
                    ...
                    >
                        <div className={classes.typeIndicator} style={{ backgroundColor: `var(--mantine-color-${getTypeColor(thread.messageType)}-6)` }} />
                        <div className={classes.messageContent} onClick={() => onSelectThread(thread)}>
                            <div className={classes.messageHeader}>
                                ...
                                <Text className={classes.messageDate}>{getRelativeTime(thread.lastMessage?.sentAt)}</Text>
                            </div>
                        ...
                        <div className={classes.bottomIndicator} style={{ backgroundColor: `var(--mantine-color-${getTypeColor(thread.messageType)}-2)` }} />
                    </Box>
                )
            })}
        </ScrollArea>
