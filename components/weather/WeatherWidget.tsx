...
import { Paper, Title, Text, Group, ActionIcon, Stack, Skeleton, Box, SegmentedControl, Flex, ScrollArea, Center } from '@mantine/core';
import { IconSun, IconCloud, IconCloudRain, IconSnowflake, IconRefresh, IconMapPin, IconClock, IconCalendar, IconCurrentLocation } from '@tabler/icons-react';
...
const WeatherWidget = () => {
    ...
    const renderHourlyView = () => {
        ...
        const startIndex = hourly.time.findIndex(timeStr => new Date(timeStr).getTime() >= now.getTime()) || 0;
            ...
            <ScrollArea type="always" scrollbarSize={8} offsetScrollbars>
                <Flex direction="row" gap="md" py="xs" style={{ flexWrap: 'nowrap', overflowX: 'auto', paddingBottom: '15px' }}>
                    {next12Hours.map((timeStr, index) => {
                         ...
                         return (
                            <Paper key={timeStr} p="sm" radius="md" withBorder style={{minWidth: '100px', textAlign: 'center'}}>
                                 <Text size="sm" fw={700} mb="xs">{hour.toLocaleTimeString([], { hour: 'numeric', hour12: true })}</Text>
                                 {hourlyWeatherIcon}
                                 <Text size="lg" fw={500}>{Math.round(hourly.temperature_2m[hourlyIndex])}°C</Text>
                                 <Text size="xs" c="blue">{hourly.precipitation_probability[hourlyIndex]}%</Text>
                            </Paper>
                         )
                    })}
                </Flex>
                ...
                <Flex direction="row" gap="md" py="xs" style={{ flexWrap: 'nowrap', overflowX: 'auto', paddingBottom: '15px' }}>
                   {daily.time.slice(0, 7).map((timeStr, index) => {
                       ...
                       const dailyWeatherIcon = getWeatherIcon(daily.weather_code[index]);
                        return (
                             <Paper key={timeStr} p="sm" radius="md" withBorder style={{minWidth: '100px', textAlign: 'center'}}>
                                 <Text size="sm" fw={700} mb="xs">{index === 0 ? 'Today' : day.toLocaleDateString([], { weekday: 'short' })}</Text>
                                 {dailyWeatherIcon}
                                 <Text size="md" fw={500}>{Math.round(daily.temperature_2m_max[index])}°/{Math.round(daily.temperature_2m_min[index])}°</Text>
                                 <Text size="xs" c="blue">{daily.precipitation_probability_max[index]}%</Text>
                            </Paper>
                        )
                   })}
                </Flex>
    ...
    const renderContent = () => {
        ...
        if (primary.error) {
            return <Text c="red" mt="md">Could not load weather data.</Text>;
        }

        if (!primary.data) {
            return <Text c="dimmed" mt="md">No weather data available for this location.</Text>;
        }
        ...
        <Paper withBorder p="lg" radius="md" style={{ flex: '1 1 400px', minWidth: '300px' }}>
            <Group justify="space-between" align="center" mb="md">
             ...
             <SegmentedControl
                ...
                data={[
                    { label: <Center><IconCurrentLocation size={16} /><Box ml="xs">Current</Box></Center>, value: 'current' },
                    { label: <Center><IconClock size={16} /><Box ml="xs">Hourly</Box></Center>, value: 'hourly' },
                    { label: <Center><IconCalendar size={16} /><Box ml="xs">Daily</Box></Center>, value: 'daily' },
                ]}
                mb="md"
            />
