...
const AppFeedbackModal: React.FC<AppFeedbackModalProps> = ({ opened, onClose }) => {
    ...
    const form = useForm({
        ...
        validate: (values) => {
            if (active === 0) {
                ...
                if (feedbackType !== 'newIdea' && !isCurrentPageRelated) return { isCurrentPageRelated: 'Please select an option' };
            }
            if (active === 1) {
    ...
    return (
        <Modal opened={opened} onClose={handleCloseModal} title="Provide App Feedback" centered size="lg">
            <Stepper active={active} onStepClick={setActive} allowNext={(active) => {
                if (active === 0) return feedbackType !== null && (feedbackType === 'newIdea' || isCurrentPageRelated !== null);
                return true;
            }}>
                ...
                <Stepper.Step label="Confirm" description="Review and submit">
                    <Text size="sm" mb="md">Please review your feedback before submitting:</Text>
                    {feedbackType === 'bug' && (
                        <>
                            <Text fw={700}>Type:</Text> <Text>Bug Report</Text>
                            <Text fw={700}>Related to current page:</Text> <Text>{isCurrentPageRelated === 'yes' ? 'Yes' : 'No'}</Text>
                            <Text fw={700}>Description:</Text> <Text>{form.values.description}</Text>
                            {form.values.expectedBehavior && <><Text fw={700}>Expected:</Text> <Text>{form.values.expectedBehavior}</Text></>}
                            {form.values.actualBehavior && <><Text fw={700}>Actual:</Text> <Text>{form.values.actualBehavior}</Text></>}
                            {form.values.stepsToReproduce && <><Text fw={700}>Steps to Reproduce:</Text> <Text>{form.values.stepsToReproduce}</Text></>}
