...
const RecipeList = ({ isRecipesListVisible, setIsRecipesListVisible, handleComponentChange }) => {
    ...
    // State for recipes, authentication, role, and meal filter
    const [user, setUser] = useState(null);
    const [role, setRole] = useState(null);
    const [recipes, setRecipes] = useState(originalRecipes);
    const [mealFilter, setMealFilter] = useState("dinner");

    // Create a filtered recipes array based on mealFilter
    const filteredRecipes = recipes.filter((recipe) => recipe.meal === mealFilter);
    ...
    // Toggle meal filter between "dinner" and "breakfast"
    const toggleMealFilter = () => {
        setMealFilter((prev) => (prev === "dinner" ? "breakfast" : "dinner"));
    };

    // SortableTable component using forwardRef and using the passed in data
    ...
    return (
        <div>
            ...
            <div style={{ marginBottom: "16px" }}>
                ...
                <button onClick={handleDownloadCSV} style={{ marginRight: "8px" }}>
                    Download CSV
                </button>
                {/* Toggle button for meal filter */}
                <button onClick={toggleMealFilter} style={{ marginRight: "8px" }}>
                    {mealFilter === "dinner" ? "Change to Breakfast" : "Change to Dinner"}
                </button>
                <button onClick={() => setIsRecipesListVisible(false)}>
                    Close Recipe List
                </button>
            </div>

            {/* Table wrapped in a printable container */}
            <div id="printable">
